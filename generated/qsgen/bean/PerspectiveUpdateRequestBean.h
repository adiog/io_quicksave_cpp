...
public:
  ...
  PerspectiveUpdateRequestBean(absl::optional<List<std::string>> ordering,
                               PerspectiveBean perspective)
      : ordering(ordering), perspective(perspective) {}

  PerspectiveUpdateRequestBean(const rapidjson::Value &value) {
    if (value.HasMember(ordering_label))
      this->ordering = absl::make_optional<List<std::string>>(
          Typoid<List<std::string>>::FromValue(value[ordering_label]));
    if (value.HasMember(perspective_label))
      this->perspective =
          Typoid<PerspectiveBean>::FromValue(value[perspective_label]);
    else
