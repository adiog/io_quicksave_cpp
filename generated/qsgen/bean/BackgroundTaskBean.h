...
public:
  ...
  BackgroundTaskBean(InternalCreateRequestBean internalCreateRequest,
                     SerializedDict kwargs, std::string name)
      : internalCreateRequest(internalCreateRequest), kwargs(kwargs),
        name(name) {}

  BackgroundTaskBean(const rapidjson::Value &value) {
    if (value.HasMember(internalCreateRequest_label))
      this->internalCreateRequest =
          Typoid<InternalCreateRequestBean>::FromValue(
              value[internalCreateRequest_label]);
    else
